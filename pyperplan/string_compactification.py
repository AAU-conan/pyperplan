...
class StringCompactifier:
    ...
    def compactify(self, strings: set[str]) -> set[str]:
        ...
        assert token_strings <= self.universe, "The given strings must be a subset of the universe."

        len_token_strings: dict[int, set[tuple[str,...]]] = {}
        for ts in token_strings:
            if len(ts) not in len_token_strings:
                len_token_strings[len(ts)] = set()
            len_token_strings[len(ts)].add(ts)

        result = set()
        for l in len_token_strings.keys():
            token_strings_l = len_token_strings[l]
            changed = True
            while changed:
                changed = False
                for ts in token_strings_l:
                    ...
                    if merge_index != -1:
                        ...
                        token_strings_l.add(ts[:merge_index] + ('|'.join(sorted(options)),) + ts[merge_index+1:])
                        changed = True
                        break
