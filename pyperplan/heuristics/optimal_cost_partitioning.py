...
class OptimalCostPartitioningHeuristic(Heuristic):
    ...
    def _initialize_lp(self):
        ...
        For each factor, we have have one variable per state, representing the goal cost of that state in that factor.
        ...
        label_factor_to_cost_var = {(l, i): LpVariable(f'{l}_in_{i}') for i in range(self.task.size()) for l in
                                    self.task.labels}
        for l in self.task.labels:
            self.prob += lpSum(label_factor_to_cost_var[(l, i)] for i in range(self.task.size())) <= self.task.label_costs[
                l], f"cost of label {l}"
        ...
        for i, factor in enumerate(self.task.factors):
            ...
            for s, l, t in factor.transitions:
                if s == t:
                    ...
                    self.prob += self.factor_state_to_goal_cost[(i,s)] <= self.factor_state_to_goal_cost[(i,t)] + label_factor_to_cost_var[ (l, i)], f'transition_{s}_{l}_{t}_in_{i}'
    ...
    def __call__(self, node: 'SearchNode') -> int:
        ...
        return math.ceil(round(self.prob.objective.value() * 100) / 100)